...
const handlCreateBoard = async (req, res) => {
    ...
    try {
    const {name, slug, description, board_category} = req.body;

    if(!name || !slug || !board_category){
    ...
    const boardCategoryDoc = await boardCategoryModel.findOne({ name: board_category });
    if(!boardCategoryDoc){
